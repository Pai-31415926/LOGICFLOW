...
// 统一从 icons.ts 引入所有图标（确保 icons.ts 里都是 export { default as xxxIcon } from '...svg'）
import {
  backwashPumpIcon,
  cartridgeFilterIcon,
...
function item(type: string, zh: string, en: string, icon?: string): PaletteItem {
  return {
    type,
    label: zh, // 左侧显示中文
    icon, // 左侧卡片图标
    properties: { deviceName: zh, deviceNameEn: en, icon }, // 节点属性
  }
}
...
export const groups: PaletteGroup[] = [
  {
    ...
    name: '机电类设备',
    items: [
      item('bpmn:motorizedValve', '电动阀门', 'Motorized Valve', motorizedValveIcon),
      item('bpmn:pump', '泵', 'Pump', pumpIcon),
      item('bpmn:backwash-pump', '反冲洗泵', 'Backwash Pump', backwashPumpIcon),
      item('bpmn:dosing-pump', '计量泵', 'Dosing Pump', dosingPumpIcon),
      item('bpmn:static-mixer', '静态混合器', 'Static Mixer', staticMixerIcon),
      item('bpmn:uf-unit', '超滤单元（UF）', 'Ultrafiltration Unit (UF)', UF_UnitIcon),
      item('bpmn:dechlorination-unit', '脱氯装置（紫外）', 'Ultraviolet (UV) Dechlorination', UV_DechlorinationIcon),
      item('bpmn:reverse-osmosis-unit', '反渗透装置（RO）', 'Reverse Osmosis (RO) Unit', RO_UnitIcon),
      item('bpmn:cartridge-filter', '滤芯过滤器', 'Cartridge Filter', cartridgeFilterIcon),
      item('bpmn:mixer', '搅拌器/混合器', 'Agitator / Mixer', mixerIcon),
      item('bpmn:water-tank', '水箱', 'Water Tank', waterTankIcon),
      item('bpmn:water-pipeline', '水管道', 'Water Pipeline', waterPipelineIcon),
      item('bpmn:chemical-tank', '药剂罐', 'Chemical Dosing Tank', chemicalTankIcon),
    ],
  },
  {
    key: 'actuating',
